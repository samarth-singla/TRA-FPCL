...
*Last updated: March 6, 2026 (v2 — Supplier + Admin Web Portal added). Hand this file to a new chat to continue work.*
...
```
lib/
├── main.dart                          ← Mobile app entry, AuthWrapper, DashboardRouter, DashboardShell
...
└── screens/
    ├── auth/
    │   ├── phone_login_screen.dart    ← Role selector (RAE/SME/SUPPLIER only — ADMIN removed) + phone input
...
### ✅ Mobile App — ADMIN Removed
- `phone_login_screen.dart`: `_roles` list is now `['RAE', 'SME', 'SUPPLIER']` (ADMIN removed)
- `main.dart` ADMIN case: shows a "Admin Portal — Web Only" message screen with Sign Out button
...
- **DashboardShell drawer** — shows hardcoded "Role: RAE" text instead of actual role
